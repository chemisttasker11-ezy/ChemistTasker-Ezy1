...
def managed_legacy(request, invoice_id, operation='detail'):
    record = get_object_or_404(InvoiceRecord, invoice_id=invoice_id, owner=request.user)
...
def detail(request, pk):
    if InvoiceRecord.objects.filter(invoice_id=pk).exists():
...
def pdf(request, invoice_id):
    if InvoiceRecord.objects.filter(invoice_id=invoice_id).exists():
