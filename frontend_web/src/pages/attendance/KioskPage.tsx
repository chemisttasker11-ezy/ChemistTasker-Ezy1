...
export default function KioskPage() {
  ...
  const [qrRevoked, setQrRevoked] = useState(false);
  ...
  const fetchQR = useCallback(async () => {
    ...
    try {
      ...
      setQrRevoked(revoked || (!desktopRuntime && err.response?.status === 401));
  ...
  // A failed QR fetch has no expiry to drive the rotation timer. Retry after
  // transient outages, while leaving revoked devices for manager re-pairing.
  useEffect(() => {
    if (!deviceToken || !qrError || qrRevoked) return;
    const interval = setInterval(fetchQR, 10000);
    return () => clearInterval(interval);
  }, [deviceToken, qrError, qrRevoked, fetchQR]);
            ...
            <Stack direction="row" spacing={1} alignItems="center">
              <Box
                sx={{
                  ...
                  height: 8,
                  borderRadius: "50%",
                  bgcolor: qrRevoked ? "#fb7185" : qrError ? "#fbbf24" : "#00bdd2",
                  boxShadow: qrError ? "none" : "0 0 10px #00bdd2",
              ...
              <Typography variant="caption" sx={{ color: "#cbd5e1", fontWeight: 500, letterSpacing: "0.02em" }}>
                {qrRevoked ? "Device revoked" : qrError ? "QR unavailable" : "Attendance Terminal Online"}
              </Typography>
            </Stack>
          </Box>
        </Stack>
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography variant="body2" color="text.secondary">
            {desktopRuntime ? (
              <>Disconnect this terminal from <strong>{pharmacyName}</strong>. ChemistTasker will revoke the device before local credentials are removed. It must be paired again to record attendance.</>
            ) : (
              <>Remove this browser kiosk from <strong>{pharmacyName}</strong>. Its registration remains active until an authorized manager revokes it in Workforce settings.</>
            )}
          </Typography>
        ...
        <DialogActions>
          ...
          >
            {settingsBusy ? <CircularProgress size={20} color="inherit" /> : desktopRuntime ? "Disconnect Device" : "Remove Browser Kiosk"}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
