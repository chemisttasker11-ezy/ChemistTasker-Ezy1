...
export const resolveSlotId = (slot: any): number | null => {
...
export const offerBelongsToSlot = (offer: any, slotId: number) => {
    ...
    if (Array.isArray(offerSlots) && offerSlots.length > 0) {
        return offerSlots.some(
            (slot: any) => resolveSlotId(slot?.slot) === slotId || resolveSlotId(slot) === slotId,
        );
    }
    const fallbackSlotId =
        resolveSlotId(offer?.slot) ?? toFiniteNumber(offer?.slot_id ?? offer?.slotId);
...
export const interestBelongsToSlot = (interest: any, slotId: number) => {
    const explicitSlotId =
        resolveSlotId(interest?.slot) ?? toFiniteNumber(interest?.slot_id ?? interest?.slotId);
