...
import { AuthProvider, useAuth } from '@/migrated/contexts/AuthContext';
...
function Ready({children}:{children:React.ReactNode}){const {isLoading}=useAuth();return isLoading?<Loading/>:<>{children}</>;}
initSharedCoreApi();
export default function PublicRuntime({page}:{page:string}) { const Component=pages[page as keyof typeof pages]; return <ThemeProvider theme={publicTheme}><AuthProvider><ToastProvider><Ready><Component/></Ready></ToastProvider></AuthProvider></ThemeProvider>; }
