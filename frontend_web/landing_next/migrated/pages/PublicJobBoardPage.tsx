...
import { Link as RouterLink, useSearchParams } from '@/public/navigation';
import {
  Alert,
  Box,
  Button,
...
export default function PublicJobBoardPage() {
  ...
  useEffect(() => {
    const title = 'Locum pharmacist shifts & pharmacy jobs | ChemistTasker';
    const description = 'Browse locum pharmacist shifts and pharmacy jobs in Australia. Filter by location and dates, then apply with your existing ChemistTasker account.';
    const origin = window.location.origin;
    const baseUrl = `${origin}/shifts/public-board`;
    const org = searchParams.get('organization');
    const canonicalUrl = baseUrl;
    const image = `${origin}/assets/pharmacist-rahul.jpg`;
      ...
      <AuthLayout title="Public Job Board" maxWidth={false} noCard showTitle={false}>
        ...
        <Dialog open={loginDialogOpen} onClose={() => setLoginDialogOpen(false)}>
          ...
          <DialogActions>
            ...
            <Button component={RouterLink} to={`/register?next=${encodeURIComponent(window.location.pathname+window.location.search)}`} variant="outlined">
              Create account
            </Button>
            <Button component={RouterLink} to={`/login?next=${encodeURIComponent(window.location.pathname+window.location.search)}`} variant="contained">
              Log in
            </Button>
          </DialogActions>
        </Dialog>
