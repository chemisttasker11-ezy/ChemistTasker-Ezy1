...
export default function AccountControls(){
 const pathname=usePathname();const {user,status,reload}=useSession();const [busy,setBusy]=useState(false),[error,setError]=useState('');
 ...
 return <div className="ct-account-controls"><a className="button primary small" href="/marketplace/new">+ Add item</a><details className="ct-account-menu"><summary>{user.first_name||'My account'}</summary><nav aria-label="Account"><span>{user.email}</span><a href="/">ChemistTasker home</a><a href="/dashboard">My dashboard</a><a href="/marketplace/new">+ Add marketplace item</a><a href="/marketplace/mine">My listings</a><a href="/marketplace/exchanges">My exchanges</a><a href="/marketplace/ethical/access">Private ethical marketplace</a><a href="/hubs">Community</a>{!!Object.keys(user.content_capabilities?.areas||{}).length&&<a href="/content">Publishing</a>}<button disabled={busy} onClick={async()=>{setBusy(true);setError('');try{await logoutSession();}catch{setError('Unable to log out. Check your connection and retry.');}finally{setBusy(false);}}}>{busy?'Logging out…':'Log out'}</button>{error&&<p role="alert">{error}</p>}</nav></details></div>;
}
