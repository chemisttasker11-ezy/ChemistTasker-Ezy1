...
import {currentSession,watchSession,type SessionUser} from './browser-session';
...
export function SessionProvider({children}:{children:React.ReactNode}){
 ...
 const reload=useCallback(async()=>{const request=++generation.current;try{const current=await currentSession();if(request!==generation.current)return;setUser(current);setStatus('authenticated');}catch(error){if(request!==generation.current)return;if((error as {status?:number}).status===401){setUser(null);setStatus('anonymous');}else setStatus('unavailable');}},[]);
