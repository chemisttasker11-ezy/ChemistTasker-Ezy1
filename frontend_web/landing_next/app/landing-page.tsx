...
function Logo({ footer = false }: { footer?: boolean }) { return <a href="#" className={`logo ${footer ? 'footer-logo' : ''}`} aria-label="ChemistTasker home"><img src="/assets/calculator-wordmark.png" alt="ChemistTasker — Pharmacy Workforce Platform" /></a>; }
...
export default function LandingPage() {
  ...
  return <>
    ...
    <main id="main">
      ...
      <section id="features" className="section container"><div className="section-heading"><div><p className="eyebrow">LESS JUGGLING. MORE CONNECTION.</p><h2>Everything your workforce needs.<br/><span className="muted-heading">Finally, in one place.</span></h2></div><p>From filling a shift to keeping your team in the loop.<br/>Make room for a more organised working day.</p></div><div className="feature-grid">{features.map(({icon:Icon,title,text,colour})=><article className="feature-card" key={title}><span className={`icon-box ${colour}`}><Icon size={23}/></span><h3>{title}</h3><p>{text}</p></article>)}</div><div className="clinical-utility"><a className="hero-calculator" href="/calculator" aria-label="Open children's dose calculator"><span className="hero-calculator-icon"><MeasuringCylinder/></span><span className="hero-calculator-copy"><span className="hero-calculator-kicker">CLINICAL UTILITY</span><strong>Children’s dose calculator</strong><span>Oral anti-infectives · Source-linked doses</span></span><ArrowUpRight size={18} aria-hidden="true"/></a></div><div className="feature-jumps"><a className="text-link" href="#all-features">Explore all capabilities <ArrowRight size={18}/></a><a className="text-link" href="#escalation">Watch the five-tier concept <ArrowRight size={18}/></a></div></section>
      <TierExplainer/>
      <FeatureCatalogue/>
      <JournalTeaser/>
      ...
      <section className="container mobile-app-section" aria-labelledby="mobile-app-title"><div className="hero-mobile-app"><div className="hero-mobile-app-intro"><span className="hero-mobile-app-icon"><Smartphone size={24}/></span><div><h2 id="mobile-app-title">Take ChemistTasker with you</h2><p>Get faster shift updates, easier scheduling and your pharmacy workflow wherever the day takes you.</p></div></div><div className="hero-mobile-app-links"><a href="https://play.google.com/store/apps/details?id=com.chemisttasker.app" target="_blank" rel="noopener noreferrer" aria-label="Download ChemistTasker on Google Play"><img src="/assets/google-play-badge.svg" alt="Get it on Google Play"/></a><a href="https://apps.apple.com/sb/app/chemisttasker/id6759088580" target="_blank" rel="noopener noreferrer" aria-label="Download ChemistTasker on the App Store"><img src="/assets/app-store-badge.svg" alt="Download on the App Store"/></a></div></div></section>
    </main>
