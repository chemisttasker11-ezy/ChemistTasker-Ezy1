...
export default function MarketplaceItem({id}:{id:string}){
 ...
 useEffect(()=>{let active=true;setState('loading');void chemistTaskerApi.marketplace.getListing(id).then(value=>{if(active){setListing(value as Listing);setState('ready');}}).catch(error=>{if(active)setState(error instanceof ApiError&&error.status===404?'missing':'error');});return()=>{active=false;};},[id,retry]);
 useEffect(()=>{if(session.status==='authenticated')void chemistTaskerApi.marketplace.getAccess().then(value=>setAccess(value as GoodsAccess)).catch(()=>{});},[session.status]);
