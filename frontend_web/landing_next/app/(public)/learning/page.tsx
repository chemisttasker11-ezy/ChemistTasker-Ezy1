...
import {safeWebUrl} from '@/lib/hub';
import styles from './learning.module.css';

type Article={id:number;title:string;slug:string;kind:'blog'|'news';topic:string;excerpt:string;read_minutes:number;published_at:string;cover_url?:string;cover_alt?:string;author_name?:string};
...
export default function LearningPage(){
 ...
 return <div className={styles.page}><section className={styles.hero}><div className="container"><p>CHEMISTTASKER LEARNING</p><GraduationCap size={38}/><h1>Learn across the pharmacy community.</h1><span>Shared resources for everyone, with role-relevant pathways when you are signed in.</span>{path&&<div className={styles.role}><strong>{options?.role_label}</strong><small>{path}</small></div>}</div></section><section className="container"><div className={styles.links}><Link href="/hubs"><Users/> Community discussions</Link><Link href="/blog"><BookOpenCheck/> Blog</Link><Link href="/news"><Newspaper/> News</Link></div><div className={styles.heading}><h2>Learning resources</h2><p>Editorial resources tagged for learning are shown here.</p></div>{articles.length?<div className={styles.grid}>{articles.map(article=>{const cover=safeWebUrl(article.cover_url);return <Link href={`/${article.kind}/${article.slug}`} key={article.id} className={styles.card}>{cover&&<div className={styles.cardImage}><img src={cover} alt={article.cover_alt||article.title} loading="lazy"/></div>}<div className={styles.cardBody}><div className={styles.cardMeta}><span>{article.kind} · {article.read_minutes} min</span>{article.author_name&&<span className={styles.authorBadge}>{article.author_name}</span>}</div><h3>{article.title}</h3><p>{article.excerpt}</p></div></Link>;})}</div>:<section className={styles.empty}><BookOpenCheck/><h3>No learning resources are published yet.</h3><p>The learning hub is ready to surface editorial resources tagged with the learning topic.</p></section>}</section></div>;
}
