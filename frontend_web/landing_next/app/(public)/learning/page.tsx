...
export default function LearningPage(){
 ...
 useEffect(()=>{void fetch('/api/platform/public-hub/articles/?topic=learning',{cache:'no-store'}).then(r=>r.ok?r.json():null).then((data:ArticleResponse|null)=>setArticles(data?.results||[])).catch(()=>{});if(session.status==='authenticated')void marketApi<Options>('me/listing-options/').then(setOptions).catch(()=>{});},[session.status]);
