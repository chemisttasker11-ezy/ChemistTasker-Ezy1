...
export default async function HubPage({params}:{params:Promise<{hub:string}>}){const {hub}=await params;if(!labels[hub])notFound();const initial=await publicFetch<Page<PublicPost>>(`community/${hub}/posts/`);if(!initial)notFound();return <CommunityFeed hub={hub} label={labels[hub]} initial={initial}/>;}
