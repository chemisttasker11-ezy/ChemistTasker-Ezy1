...
import {ethicalApi,type EthicalContext} from './api';
...
export default function EthicalWorkspaceV2({view}:{view:View}){
 ...
 useEffect(()=>{
  ...
  setLoading(true);
  ethicalApi<{contexts:EthicalContext[]}>('me/access/').then(data=>{
 ...
 const reload=useCallback(async()=>{
  ...
  try{
   let result:unknown[]=[];
   if(view==='inventory'){
    result=await ethicalApi<Lot[]>(`inventory/lots/?pharmacy=${selected}`);
   }else if(view==='transfers'){
    result=await ethicalApi<Transfer[]>(`transfers/?pharmacy=${selected}`);
   }else if(view==='listings'||view==='new'){
    const data=await ethicalApi<{results:Listing[]}>(`me/listings/?pharmacy=${selected}`);
    result=data.results||[];
   }else{
    const data=await ethicalApi<{results:Listing[]}>(`listings/?pharmacy=${selected}`);
    result=data.results||[];
   }
...
function Access({context}:{context:EthicalContext}){const [message,setMessage]=useState('');const canApply=context.blockers.some(row=>row.code==='PHARMACY_APPROVAL_REQUIRED');return <div className="workspace-stack">{canApply&&<form className="workspace-form compact" onSubmit={async e=>{e.preventDefault();try{await ethicalApi(`pharmacies/${context.pharmacy.id}/approval/`,'POST',new FormData(e.currentTarget));setMessage('Premises application submitted for private review.')}catch(error){setMessage((error as Error).message)}}}><fieldset><legend>Premises application</legend><label><span>PBS/PAN approval number</span><input name="pbs_approval_number" required/></label><label><span>Business phone</span><input name="business_phone" required/></label><label><span>Business email</span><input name="business_email" type="email" required/></label><label className="full"><span>Private evidence</span><input name="evidence" type="file" required/></label></fieldset><button className="button primary">Submit for review</button></form>}{message&&<p role="status">{message}</p>}</div>}

function Inventory({pharmacy,lots,reload,setError}:{pharmacy:number;lots:Lot[];reload:()=>Promise<void>;setError:(v:string)=>void}){const [barcode,setBarcode]=useState('');const [product,setProduct]=useState<Product>();const [batch,setBatch]=useState<{id:number;row_count:number}>();const [message,setMessage]=useState('');async function lookup(){try{const found=await ethicalApi<Product|Record<string,never>>(`catalogue/lookup/?pharmacy=${pharmacy}&barcode=${encodeURIComponent(barcode)}`);if('id' in found){setProduct(found as Product);setMessage('Reviewed private product matched.')}else setMessage('No approved product matched.')}catch(error){setError((error as Error).message)}}return <div className="workspace-stack"><section className="workspace-panel"><h2>Scan or enter a medicine barcode</h2><div className="inline-upload"><input value={barcode} onChange={e=>setBarcode(e.target.value)} placeholder="Barcode"/><button onClick={()=>void lookup()}><ScanLine size={14}/> Lookup</button></div>{product&&<p><strong>{product.name}</strong> · {product.strength} · {product.form} · {product.schedule}</p>}</section><form className="workspace-form compact" onSubmit={async e=>{e.preventDefault();const data=new FormData(e.currentTarget);data.set('pharmacy',String(pharmacy));data.set('source_rights_attested','true');try{const staged=await ethicalApi<{id:number;row_count:number}>('inventory/imports/','POST',data);setBatch(staged);setMessage(`${staged.row_count} row(s) staged.`)}catch(error){setError((error as Error).message)}}}><fieldset><legend>Import inventory CSV</legend><label><span>Source name</span><input name="source_name" required/></label><label><span>CSV file</span><input name="file" type="file" accept=".csv,text/csv" required/></label></fieldset><button className="button secondary"><FileUp size={15}/> Stage CSV</button></form>{batch&&<section className="workspace-panel"><p>{batch.row_count} row(s) staged. No stock changes until commit.</p><button className="button primary" onClick={async()=>{try{const r=await ethicalApi<{status:string;accepted_count:number;rejected_count:number}>(`inventory/imports/${batch.id}/commit/`,'POST',{});setMessage(`${r.status}: ${r.accepted_count} accepted, ${r.rejected_count} rejected`);await reload()}catch(error){setError((error as Error).message)}}}>Commit reconciled inventory</button></section>}<section className="workspace-section"><h2>Current private lots</h2>{lots.map(lot=><article key={lot.id}><strong>{lot.product_detail.name} {lot.product_detail.strength}</strong><p>Batch {lot.batch_number} · exp {lot.expiry_date} · {lot.available_quantity} available</p></article>)}</section>{message&&<p role="status">{message}</p>}</div>}
...
function Mine({rows,isOwner,reload,setError}:{rows:Listing[];isOwner:boolean;reload:()=>Promise<void>;setError:(v:string)=>void}){
 async function act(row:Listing,action:string,extra:Record<string,unknown>={}){
  try{
   await ethicalApi(`listings/${row.id}/${action}/`,'POST',{expected_version:row.version,...extra});
   await reload();
  }catch(error){
...
function Transfers({rows,reload,setError}:{rows:Transfer[];reload:()=>Promise<void>;setError:(v:string)=>void}){
 async function act(row:Transfer,action:string){
  try{
   await ethicalApi(`transfers/${row.id}/${action}/`,'POST',{expected_version:row.version,client_request_id:crypto.randomUUID()});
   await reload();
  }catch(error){
   setError((error as Error).message);
  }
