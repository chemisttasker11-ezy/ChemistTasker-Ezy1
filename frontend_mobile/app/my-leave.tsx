...
export default function MyLeaveScreen() {
  ...
  const load = useCallback(async () => { try { const rows = await workforce.listLeave(); setRows(Array.isArray(rows) ? rows : []); } catch (e: any) { setError(e?.payload?.error || e?.message || 'Unable to load leave.'); } }, []);
  ...
  const submit = async () => {
    ...
    setError('');
    try {
      await workforce.createLeave({ membership_id: membershipId, leave_type: type, start_at: start.toISOString(), end_at: end.toISOString(), note });
      setVisible(false); setNote(''); await load();
    } catch (e: any) { setError(e?.response?.data?.error || e?.message || 'Unable to request leave.'); }
  };
  const cancel = async (id: number) => { try { await workforce.decideLeave(id, 'CANCELLED', 'Cancelled by worker'); await load(); } catch (e: any) { setError(e?.payload?.error || e?.message || 'Unable to cancel request.'); } };
