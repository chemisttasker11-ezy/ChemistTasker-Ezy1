  ...
  return <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, backgroundColor: theme.colors.background }}>
    ...
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 16, paddingBottom: 40, gap: 16 }}>
      ...
      {value.lines.map((line, index) => <Surface key={`${index}-${line.item_id}`} elevation={0} style={{ padding: 16, borderRadius: 8, borderWidth: 1, borderColor: theme.colors.outlineVariant }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}><Text variant="titleSmall" style={{ flex: 1 }}>Item {index + 1}</Text><IconButton icon="delete-outline" accessibilityLabel={`Remove item ${index + 1}`} onPress={() => change('lines', value.lines.filter((_, i) => i !== index))} /></View>
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginVertical: 12 }}>{(['quantity', 'unit_price', 'discount'] as const).map((key, i) => <TextInput key={key} mode="outlined" dense style={{ flex: 1 }} keyboardType="decimal-pad" label={['Qty', 'Rate', 'Disc %'][i]} value={line[key]} onChangeText={text => changeLine(index, { [key]: text })} />)}</View>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>{(['GST', 'GST_FREE', 'INPUT_TAXED', 'OUT_OF_SCOPE'] as const).map((tax, i) => <Chip key={tax} selected={line.tax_code === tax} onPress={() => changeLine(index, { tax_code: tax })}>{['GST 10%', 'GST-free', 'Input taxed', 'No GST'][i]}</Chip>)}</View>
