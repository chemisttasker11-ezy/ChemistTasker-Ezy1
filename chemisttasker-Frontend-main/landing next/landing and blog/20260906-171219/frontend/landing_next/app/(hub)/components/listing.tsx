...
export default async function Listing({ kind, filters }: { kind: Kind; filters: SearchParams }) {
  ...
  return <>
    <section className="hub-hero"><div className="container hub-hero-grid"><div><p className="eyebrow"><span className="live-dot"/> {news ? 'THE PUBLIC PHARMACY HUB' : 'THE CHEMISTTASKER JOURNAL'}</p>
      <h1>{news ? <>Stay informed.<br/><span>Stay connected.</span></> : <>Fresh perspectives.<br/><span>Better pharmacy days.</span></>}</h1>
      <p className="hub-hero-description">{news ? 'The updates that matter to Australian pharmacy. Follow industry news, explore career opportunities, and add your voice to the conversation.' : 'Ideas, practical guides, and stories for the people behind every pharmacy. A little knowledge for your next chapter.'}</p>
    ...
    <div className="container hub-content"><nav className="hub-section-nav" aria-label="Publication sections"><Link href="/blog" aria-current={!news ? 'page' : undefined}><BookOpen size={18}/>The blog</Link><Link href="/news" aria-current={news ? 'page' : undefined}><Radio size={18}/>News & community</Link><span>Knowledge grows when we share it.</span></nav>
      ...
      <section className="hub-join"><span className="icon-box purple"><MessageCircle size={27}/></span><div><p className="eyebrow">MORE THAN A COMMENT SECTION</p><h2>A place for your perspective.</h2><p>Ask a question. Share your experience. Connect with people who understand your working day.</p></div><a href={`${platform}/register`} className="button primary">Join the conversation <ArrowUpRight size={18}/></a></section>
    </div>
  </>;
}
