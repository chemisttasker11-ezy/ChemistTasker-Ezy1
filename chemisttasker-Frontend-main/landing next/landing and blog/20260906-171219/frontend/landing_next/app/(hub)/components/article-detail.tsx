...
export default async function ArticleDetail({ slug, kind }: { slug: string; kind: Kind }) {
  ...
  const headings = sections.map((text, index) => ({ text, index })).filter(({ text }) => text.startsWith('## '));
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData).replace(/</g, '\\u003c') }}/>
    <div className="container hub-article-shell"><nav className="hub-breadcrumb" aria-label="Breadcrumb"><Link href="/">Home</Link><span>/</span><Link href={`/${kind}`}>{kind === 'news' ? 'News & community' : 'The blog'}</Link><span>/</span><span aria-current="page">{topics[article.topic]}</span></nav>
      <header className="hub-article-heading"><Link className="hub-back" href={`/${kind}`}><ArrowLeft size={17}/>Back to {kind === 'news' ? 'news' : 'the journal'}</Link><span className="hub-tag">{topics[article.topic]}</span><h1>{article.title}</h1><p>{article.excerpt}</p>
        <div className="hub-article-meta"><div className="hub-byline"><span className="hub-avatar">{article.author_name.split(' ').map(w => w[0]).slice(0, 2).join('')}</span><div><strong>{article.author_name}</strong><span><time dateTime={article.published_at}>{dateLabel(article.published_at)}</time></span></div></div><span><Clock3 size={17}/>{article.read_minutes} min read</span><a href="#discussion"><MessageCircle size={17}/>Join the discussion</a><ShareButton/></div>
      </header><ArticleArt article={article} large/>
      <div className="hub-reading-layout"><aside className="hub-reading-aside"><p className="eyebrow">IN THIS STORY</p>{headings.length ? <nav aria-label="Article sections">{headings.map(({ text, index }) => <a href={`#section-${index}`} key={index}>{text.slice(3)}</a>)}</nav> : <p>A perspective from the ChemistTasker journal.</p>}<div className="hub-reading-note"><MessageCircle size={23}/><h3>Good conversations start with you.</h3><p>Have a perspective to add? Meet us in the discussion.</p><a href="#discussion">Add your voice <ArrowUpRight size={16}/></a></div></aside>
        <div><article className="hub-prose">{sections.map((text, index) => text.startsWith('## ') ? <h2 id={`section-${index}`} key={index}>{text.slice(3)}</h2> : <p key={index}>{text}</p>)}</article>
