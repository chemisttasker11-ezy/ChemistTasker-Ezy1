...
import { Article, articlePath, dateLabel, HubComment, Page, Reactions, platform } from '../../../lib/hub';
...
export default function Discussion({ article }: { article: Article }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const user = await api<{ name: string }>('session/').catch(() => null);
        const current = await api<Article>(`articles/${article.slug}/`);
        const data = await api<Page<HubComment>>(`articles/${article.slug}/comments/`);
        if (active) { setMember(user); setReactions(current.reactions); setCommentsOpen(current.comments_open); setItems(data.results); setMore(!!data.next); }
  ...
  return <section className="hub-discussion" id="discussion" aria-labelledby="discussion-heading"><p className="eyebrow">YOUR PERSPECTIVE MATTERS</p><h2 id="discussion-heading">Let’s talk about it.</h2><p>Keep it thoughtful, respectful, and free of personal patient information.</p>
    ...
    {loading ? <p role="status">Loading the conversation…</p> : <>{!commentsOpen ? <div className="hub-sign-in-prompt"><strong>This discussion is closed.</strong><p>You can still read the conversation and react.</p></div> : member ? <><div className="hub-member-line"><span>Posting as <strong>{member.name}</strong></span><button className="hub-text-button" disabled={busy} onClick={async () => { setBusy(true); try { await api('session/', 'DELETE'); setMember(null); await refresh(); const current = await api<Article>(`articles/${article.slug}/`); setReactions(current.reactions); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }}>Sign out</button></div><Composer label="Post comment" submit={async body => { await api(`articles/${article.slug}/comments/`, 'POST', { body }); setNotice('Your comment has been posted.'); try { await refresh(); } catch { setError('Your comment was saved, but the conversation could not refresh. Reload to see it.'); } }}/></> : <div className="hub-sign-in-prompt"><span className="icon-box purple"><MessageCircle/></span><div><h3>Be part of the conversation.</h3><p>Log in with your ChemistTasker account to comment, reply, and react. Every role is welcome.</p><div className="button-row"><Link className="button primary small" href={signIn}>Log in to join <ArrowUpRight size={16}/></Link><a className="button secondary small" href={`${platform}/register`}>Create an account</a></div></div></div>}
